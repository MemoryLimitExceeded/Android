...
- ##### View 树
  ...
  - ###### 遍历时机
    ...
    - ViewRootImpl.invalidate() 系列
      ...
      @Override
      public void invalidateChild(View child, Rect dirty)	//Rect 可以看做是屏幕上一小块矩形
      @Override
      ...
      1. onDescendantInvalidated() 会调用 invalidate()。
      2. invalidateWorld() 对传入 View 这个实例所在的子树的所有节点都调用了 View.invalidate()。
      3. invalidateChild() 直接调用 invalidateChildInParent(null, dirty)。
      4. invalidateChildInParent() 调用 invalidateRectOnScreen()。
      5. invalidate() 和 invalidateRectOnScreen() 都能调用到 scheduleTraversals()，也就是遍历的入口。
