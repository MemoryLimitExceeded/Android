...
- ##### View 树
  ...
  - ###### 遍历时机
    ...
    - ViewRootImpl.invalidate() 系列
      ...
      > ※ invalidate() 一系列方法很多，这里就只放与之关系比较大的方法，下同。
  
    - ViewGroup.invalidate() 系列
  ...
  - ###### 遍历流程
    ...
    目前知道遍历的入口，就从 scheduleTraversals() 开始：
    ...
    看来 View 树的遍历是以 Runnable 的方式启动的。
  
    > ※ mChoreographer.postCallback() 的调用链太长这
